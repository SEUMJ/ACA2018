...
Preprocess p;       // 预处理类对象
// FC Buffers
float fc_dati_buf[FULL_SIZE * FC_DATA_WIDTH];
float fc_res_buf[FULL_SIZE];
// FM Buffers
float dense_res_buf[FULL_SIZE];
float sparse_res_buf[FULL_SIZE];



// Functions
void data_gen(ifstream& infile);            // Prepare data for FPGA
void gemmcpy(int dir, int fid);             // Copy data from CPU memory to FPGA memory.
...
int main(int argc, char* argv[])
{
    ...
    data_gen(infile);
    clock_t et = clock();
    float pt = (float)(et - st) / CLOCKS_PER_SEC;
    cout << "Preprocessing time: " << pt << "s" << endl;
    ...
    {
        // st = clock();
        gemmcpy(0, round);      // Copy data from CPU memory to FPGA memory.
        // et = clock();
        // cpt += (float)(et - st) / CLOCKS_PER_SEC;

        // st = clock();
        for (int i = 0; i < CORE_NUM; i++) gemm_hw(i);
        // et = clock();
        // gmt += (float)(et - st) / CLOCKS_PER_SEC;

        // st = clock();
        gemmcpy(1, round);      // Copy result from FPGA memory to CPU memory.
        // et = clock();
        // cpt += (float)(et - st) / CLOCKS_PER_SEC;

        cout << "Round " << 1 + round++ << " finished.\n";
    }
    // cout << "Polaris memcpy time: " << cpt << "s" << endl;
    // cout << "Polaris sgemm time: " << gmt << "s" << endl;
...
void data_gen(ifstream& infile)
{
    ...
    // FC Data Preparation
    for (int db_cnt = 0; db_cnt < FULL_SIZE; db_cnt ++)
    {
...
// Copy data from CPU memory to FPGA memory.
inline void gemmcpy(int dir, int fid)
{
    if (dir == 0)       // Copy FC input data from CPU memory to FPGA memory.
    {
        polaris_memcpy(ctxt, POLARIS_HOST_TO_DEVICE, fc_L0i_fpga,
                            &fc_dati_buf[FRACTION_SIZE * FC_DATA_WIDTH * fid],
                            FRACTION_SIZE * FC_DATA_WIDTH * sizeof(float));
    }
    else                // Copy data from FPGA memory to CPU memory.
    {
        polaris_memcpy(ctxt, POLARIS_DEVICE_TO_HOST,
                            &fc_res_buf[FRACTION_SIZE * fid],
                            fc_L3O_fpga, FRACTION_SIZE * sizeof(float));
    }
}
...
void gemm_hw(int sid)
{
    ...
    // FC Layer 1
    polaris_gemm(ctxt, CORE_DAT_SIZE, FC_SIZE, FC_SIZE, fc_L1i_fpga, fc_L1w_fpga,
                    fc_L2i_fpga, fc_L1b_fpga);
    polaris_activation(ctxt, POLARIS_RELU, CORE_DAT_SIZE * FC_SIZE,
                    1, fc_L2i_fpga, 0, fc_L2i_fpga);
    
    // FC Layer 2
    polaris_gemm(ctxt, CORE_DAT_SIZE, FC_SIZE, FC_SIZE, fc_L2i_fpga, fc_L2w_fpga,
                    fc_L3i_fpga, fc_L2b_fpga);
    polaris_activation(ctxt, POLARIS_RELU, CORE_DAT_SIZE * FC_SIZE,
                    1, fc_L3i_fpga, 0, fc_L3i_fpga);
    
    // FC Layer 3
    polaris_gemm(ctxt, CORE_DAT_SIZE, 1, FC_SIZE, fc_L3i_fpga, fc_L3w_fpga,
                    &fc_L3O_fpga[CORE_DAT_SIZE * sid], fc_L3b_fpga);
}
...
inline void deep_fm_malloc()
{
    ...
    polaris_malloc(ctxt, CORE_DAT_SIZE * FC_SIZE * sizeof(float), (void**)&fc_L1i_fpga);
    polaris_malloc(ctxt, CORE_DAT_SIZE * FC_SIZE * sizeof(float), (void**)&fc_L2i_fpga);
    polaris_malloc(ctxt, CORE_DAT_SIZE * FC_SIZE * sizeof(float), (void**)&fc_L3i_fpga);
